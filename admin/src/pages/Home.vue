...
const invitingClass = ref<Class | null>(null)
function openInviteModal() {
  if (!currentClass.value) return
  invitingClass.value = currentClass.value
  showInviteModal.value = true
}
function onInviteUpdated(code: string) {
  if (invitingClass.value) invitingClass.value = { ...invitingClass.value, invite_code: code }
  if (currentClass.value && currentClass.value.id === invitingClass.value?.id) {
    currentClass.value = { ...currentClass.value, invite_code: code }
  }
}
...
<template>
  <div class="min-h-screen bg-[#fffdfb] font-sans text-slate-800">
        ...
        <div class="flex min-w-0 flex-col gap-5" :class="stampMode ? 'pb-44 xl:pb-0' : ''">
          <header class="relative flex min-h-[62px] flex-wrap items-center gap-3">
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              <button type="button" aria-label="邀请家长" class="inline-flex h-[42px] w-[42px] items-center justify-center rounded-xl border border-orange-200 bg-white text-sm font-semibold text-[#b85e25] shadow-sm lg:w-auto lg:gap-1.5 lg:px-[13px]" :class="stampDimClass" v-if="currentClass && currentClass.id !== DEMO_CLASS_ID" @click="openInviteModal"><span class="material-symbols-rounded text-[18px] leading-none">badge</span><span class="hidden lg:inline">邀请家长</span></button>
    ...
    <ClassInviteModal
      v-if="showInviteModal && invitingClass"
      :class-id="invitingClass.id"
      :class-name="invitingClass.name"
      :code="invitingClass.invite_code || ''"
      :link="invitingClass.inviteLink || ''"
      @close="showInviteModal = false"
      @updated="onInviteUpdated"
    />
